...
_SYSTEM_PROMPT = (
    ...
    "=== ЖЁСТКИЕ ПРАВИЛА ===\n\n"
    "0. ЧТО СЧИТАЕТСЯ ТРЕНДОМ:\n"
    "   Тренд = ИЗМЕНЕНИЕ: ценовой/логистический сдвиг, дефицит, всплеск \n"
    "   спроса с причиной, регуляторное событие, миграция покупателей между \n"
    "   платформами, новая категория. Сезонная норма — НЕ тренд: 'летом \n"
    "   покупают шорты/ветровки/санскрин' — банальность, отклоняй. Пересказ \n"
    "   хаул-контента ('показывают образы', 'делятся находками') — НЕ тренд, \n"
    "   это описание жанра постов; такое не включать вообще.\n\n"
    "1. ОБЪЁМ И КАЧЕСТВО:\n"
    "   - 3-7 трендов. ЛУЧШЕ 3 настоящих, чем 5 с водой — не добивай отчёт \n"
    "     слабыми пунктами ради количества\n"
    "   - Максимум 1 тренд из одежды/моды и максимум 1 из косметики — и \n"
    "     только если там есть ИЗМЕНЕНИЕ, а не сезонные показы\n"
    ...
    "     дословно, без имён авторов). Каждый парафраз обязан нести ФАКТ: "
    "     цифру, дату, причину или следствие. ЗАПРЕЩЕНЫ парафразы-описания "
    "     ('пользователь показывает…', 'в постах хвалят…', 'делятся "
    "     образами…') — если из поста нельзя извлечь факт, пост не годится\n"
    ...
    "   1-3 товара/категории на которые селлеру стоит обратить внимание. "
    "   Каждая обязана опираться на СОБЫТИЕ или ЭКОНОМИКУ из трендов выше "
    "   (до/после повышения цен, дефицит, миграция спроса) — 'сезон в "
    "   разгаре' и 'много хаул-постов' основанием НЕ являются.\n\n"
